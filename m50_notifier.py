...
class Notifier:
    ...
    def 카톡으로보내기(self, s_선정로직):
        ...
        s_기준정보 = f'# {n_회차}회 - {s_추첨일_dot}추첨 #\n  ({"_".join(s_선정로직.split("_")[1:])})'
        s_메세지 = s_기준정보 + '\n' + s_선정번호
...
if __name__ == '__main__':
    n = Notifier()
    # n.카톡으로보내기(s_선정로직='번호선정_1개2개연계_최빈수연계')
    n.카톡으로보내기(s_선정로직='번호선정_2개번호_따라가기')
    # n.카톡으로보내기(s_선정로직='번호선정_2개번호_따라가기_확률반영')
    # n.카톡으로보내기(s_선정로직='번호선정_2개번호_최빈수')
    # n.카톡으로보내기(s_선정로직='번호선정_2개번호_최빈수_중복제외')
    # n.카톡으로보내기(s_선정로직='번호선정_복합로직_최빈수_따라가기')
