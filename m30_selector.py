...
if __name__ == '__main__':
    ...
    # s = Selector(s_기준일='20230331', b_test=True)
    s = Selector(s_기준일=None)
    ...
    # 과거 데이터 생성용 실행 코드
    # li_기준일 = [pd.Timestamp('20180105') + pd.Timedelta(days=7*n) for n in range(281)]
    # li_기준일 = [dt.strftime('%Y%m%d') for dt in li_기준일]
    #
    # from tqdm import tqdm
    # for s_기준일 in tqdm(li_기준일):
    #     s = Selector(s_기준일=s_기준일)
    #     s.번호선정(s_선정로직='번호선정_2개번호_최빈수')
    #     s.번호선정(s_선정로직='번호선정_2개번호_최빈수_중복제외')
    #     s.번호선정(s_선정로직='번호선정_2개번호_따라가기')
    #     s.번호선정(s_선정로직='번호선정_2개번호_따라가기_확률반영')
